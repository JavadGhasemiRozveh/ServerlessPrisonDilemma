...
def present_value(payoff, discount_factor, time_step):
    return payoff * (discount_factor ** time_step)


# Dynamic Discount Factor based on Cooperation History
def calculate_delta(history):
    if len(history) == 0:
        return 0.5  # Default starting value
    cooperation_count = history.count('C')
    return cooperation_count / len(history)  # More cooperation → Higher Delta
    

# Updated Payoff Functions
def client_payoff(client_action, service_provider_action, client_history, time_step):
    base_payoff = 0
    if client_action == 'C' and service_provider_action == 'C':
        base_payoff = xq - phi
    elif client_action == 'D' and service_provider_action == 'C':
        base_payoff = xq - phi_prime
    elif client_action == 'C' and service_provider_action == 'D':
        base_payoff = xq_prime - phi
    elif client_action == 'D' and service_provider_action == 'D':
        base_payoff = xq_prime - phi_prime
    delta = calculate_delta(client_history)
    return present_value(base_payoff, delta, time_step)

def serviceprovider_payoff(client_action, service_provider_action, provider_history, time_step):
    base_payoff = 0
    if client_action == 'C' and service_provider_action == 'C':
        base_payoff = phi - mu_q
    elif client_action == 'D' and service_provider_action == 'C':
        base_payoff = phi_prime - mu_q
    elif client_action == 'C' and service_provider_action == 'D':
        base_payoff = phi - mu_q_prime
    elif client_action == 'D' and service_provider_action == 'D':
        base_payoff = phi_prime - mu_q_prime
    delta = calculate_delta(provider_history)
    return present_value(base_payoff, delta, time_step)
...
for i, (serviceprovider_strategy_name, serviceprovider_strategy_func) in enumerate(strategies.items()):
    for j, (client_strategy_name, client_strategy_func) in enumerate(strategies.items()):
        ...
        for hour in range(simulation_duration):
            ...
            this_client_payoff = client_payoff(client_action , serviceprovider_action ,client_history , hour)
            this_serviceprovider_payoff = serviceprovider_payoff(client_action , serviceprovider_action,serviceprovider_history , hour)
